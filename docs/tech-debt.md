...
## [RESOLVIDO 2026-08-26] "Nova Transação"/"Nova Voucher" — select de Vendedor (Associado) não mostrava a seleção visualmente
`TransaçãoCadastrar.jsx` e `VoucherCadastrar.jsx` — o `<select>` de Vendedor comparava um `value` que nunca batia com o `value` das `<option>` geradas por `AssociadosDiretorioOptions` (que usa o objeto inteiro serializado, `JSON.stringify(item)`, como valor da opção — mesmo padrão reaproveitado no cadastro de Voucher). Em `TransaçãoCadastrar.jsx`, o select comparava `vendedor?.id` (um UUID) contra esses valores serializados — nunca batia; em `VoucherCadastrar.jsx`, comparava o objeto `vendedor` direto (nem é string). Resultado: ao clicar numa opção, o estado interno atualizava certo, mas o `<select>` visualmente "voltava" pra Selecione — parecia que não deixava escolher (o formulário até submetia com o valor certo se o usuário confiasse e enviasse mesmo assim, mas a experiência parecia quebrada). Achado pelo usuário via captura de tela comparando com o fluxo de Agência (que funciona, porque `AgenciasOptions` usa `item.id` puro como valor — comparação direta bate).

**O que mudou:** `TransaçãoCadastrar.jsx` — `value` do select de Vendedor passa a comparar de acordo com o `vendedorTipo` (`vendedor?.id` pra Agência, `JSON.stringify(vendedor)` pra Associado — mesmo formato que a option usa). `VoucherCadastrar.jsx` — mesma correção (`JSON.stringify(vendedor)`).

**Validado:** `npx tsc --noEmit` limpo; `npm test` 32/32; `npm run build` sem erro; contra API/Postgres reais em Docker + Playwright — confirmado que a opção escolhida permanece visualmente selecionada após o clique (antes: sempre voltava pra "Selecione"); transação de teste enviada de ponta a ponta com `vendedorId` correto na resposta (201).

## [RESOLVIDO 2026-08-26] Tela "Associados" (marketplace) vazia pra quem não é Matriz/agency_admin
`Associados.jsx` (marketplace público de associados, cards com foto/categoria/avaliação/contato) usava `useQueryAssociados()` → `GET /associados`, endpoint **administrativo** (`roleGuard('superadmin', 'agency_admin')` — nem `associate_admin`/`operator` nem `agency_operator` têm acesso). Pra qualquer Associado logado, a chamada sempre voltava 403 e a tela ficava sem nenhum card, sem erro visível. Achado pelo usuário comparando o mesmo print logado como Matriz (cards aparecem) vs Associado (vazio).

**O que mudou:** `Associados.jsx` passou a usar `useQueryAssociadosDiretorio()` (`GET /associados/diretorio`) — já era acessível a qualquer role autenticada (`associate_admin/operator`, `agency_admin/operator`, `superadmin`) e já excluía dados financeiros de propósito. `associate.service.ts::listDiretorio()` ganhou os campos que o card precisa exibir (antes só tinha o mínimo pra um dropdown de seleção): `status`, `descricao`, `imagemUrl`, `categoriaId`, `contatos`, `agencia.nome`, `gerente.nome`, `conta.numero` — deliberadamente **sem** `conta.saldo`/`limiteCredito` (só quem chama `GET /associados`, admin/superadmin, continua vendo dado financeiro).

**Achado incidental corrigido também:** ao investigar, o card "Associados" e "Fundo Permuta" do dashboard (ajustados numa rodada anterior) chamavam `useQueryAssociados()`/`useQueryMeusAssociados()` **sem nenhuma guarda de role** — pra um Associado comum logado, isso gerava 403 em loop no carregamento do dashboard mesmo sem nunca visitar a tela de Associados. Corrigido com o mesmo padrão `enabled` já usado em outros lugares (`useQueryAssociados`/`useQueryMeusAssociados` ganharam parâmetro `enabled`; novo helper `podeListarTodosAssociados()` em `getId.js` espelha o guard real do backend). Mesmo problema existia em `AssociadoOptions.jsx` (usado nas buscas de Transações — geraria 403 em loop pra qualquer Associado usando essas telas), corrigido junto.
...
## [RESOLVIDO 2026-08-26] Dashboard não atualizava saldo/limite após crédito aprovado (ou qualquer outra mudança de saldo)
`LimiteCredito.jsx`/`SaldoPermutas.jsx` (cards "Resumo Financeiro" da home) liam `state.user.conta.limiteCredito`/`saldoPermuta` — um snapshot buscado **uma única vez** no login (`/auth/me`, via `useQueryLogin` no `App.jsx`, montado uma vez na raiz do app) e nunca mais atualizado depois. Qualquer ação que muda o saldo (crédito aprovado, permuta, negociação, transferência...) deixava esses dois números visualmente congelados até o usuário deslogar e logar de novo — sem nenhum feedback de que a ação funcionou. Achado pelo usuário: aprovou um crédito na Matriz, viu confirmado, mas o Associado continuava vendo o limite antigo no dashboard.

**O que mudou:** `GET /extrato/saldo` (`report.service.ts::saldo()`) ganhou `limiteCredito` no select (antes só `saldo`/`numero`). Novo hook `useQuerySaldoConta` busca esse endpoint via React Query — sempre refaz a busca quando o componente monta (volta pra home) ou a janela ganha foco, nunca depende do snapshot velho do login. `LimiteCredito.jsx`/`SaldoPermutas.jsx` reescritos pra usar esse hook em vez de `state.user.conta`.

**Fora do escopo desta rodada:** `calcularDisponibilidade.js` (validação de saldo disponível em algum formulário de compra) ainda lê o snapshot antigo de `state.user.conta` — o backend revalida saldo/limite de forma autoritativa em toda operação financeira de qualquer forma (`saldoSuficienteParaDebito`), então não é um risco de segurança, só uma pré-validação de front que pode estar desatualizada em casos raros. Registrado aqui, não corrigido agora.

**Validado:** `npx tsc --noEmit` limpo; `npm test` 32/32; `npm run build` sem erro; contra API/Postgres reais em Docker + Playwright — replicado o cenário exato do usuário: Associado logado vê saldo (RT$ 10.472,34), Matriz aprova um crédito de RT$ 777 via API **sem o Associado nunca deslogar**, Associado só navega pra outra tela e volta pra home — saldo atualizado corretamente pra RT$ 11.249,34 (777 a mais, exato).

## [Decisão de produto 2026-08-26] "Crédito" reorganizado: aumenta limite de crédito, não injeta saldo
Discussão iniciada por confusão real: o dashboard mostrava "Limite de Crédito" parado (20.000) e "Saldo em Permutas" maior (21.000) depois de créditos aprovados, e o usuário notou o que parecia ser dois campos de limite conflitantes. Análise: não eram dois campos conflitando — eram dois conceitos genuinamente diferentes (limite = teto de quanto pode ficar negativo, fixado no cadastro; crédito aprovado = injeção de saldo, fluxo separado), documentados como tal desde o início. Só que essa separação não fazia sentido de negócio pro usuário: "Crédito" deveria ser a Matriz liberando **mais limite pra comprar**, não dando RT de graça — o associado só gera RT de verdade quando compra e fica negativo, igual a qualquer outra operação do sistema.

**O que mudou:** `credito.service.ts::finalizarCredito()` — aprovar uma `SolicitacaoCredito` agora incrementa `limiteCredito` (sincronizado em `Associado`, fonte editável no cadastro, **e** `Conta`, usado na validação de saldo/CHECK do banco — mesmo padrão de `PUT /associados/:id`), em vez de criar `MovimentacaoConta` e incrementar `saldo`. Nenhuma movimentação de saldo acontece mais nesse fluxo — não é mais uma operação financeira, é um ajuste de teto de risco. `GET /relatorios/emissao-matriz` — a categoria antes chamada `creditoAprovado` (que contava como "emissão de RT") virou `limiteAprovado`, informativa (como `compraMatriz`), removida da fórmula de `emissaoLiquida` (agora `injecaoDireta − queima`) — aprovar limite não cria RT nenhum na hora, só quando o associado de fato compra usando esse espaço (contabilizado normalmente como permuta/negociada). Front: labels atualizados pra "Aumento de Limite" em vez de "Valor"/"RT$" em `CreditoSolicitar.jsx`, `CreditosModal.jsx`, `constantCreditos.js` (afeta as 4 telas que reaproveitam esse componente/constante — Solicitar, Meus, Analisar, Aprovar).

**Validado:** `npx tsc --noEmit` limpo; `npm test` 32/32; `npm run build` sem erro; contra API/Postgres reais em Docker + Playwright — solicitado aumento de RT$500, aprovado pela Matriz: `Associado.limiteCredito` e `Conta.limiteCredito` confirmados 2.000→2.500 em sincronia; `Conta.saldo` confirmado **inalterado** (11.249,34 antes e depois); confirmado nenhuma nova linha em `movimentacao_conta` pra esse crédito; dashboard do Associado confirmado mostrando "Limite de Crédito: RT$ 2.500,00" (mudou) e "Saldo em Permutas: RT$ 11.249,34" (não mudou), refletindo corretamente a nova semântica.

## [Decisão de produto 2026-08-26] "Crédito" (aumento de limite) estendido para Agências
Sequência direta da decisão anterior: Agência também compra e vende (ver `AJUSTES.md` 2026-08-13), então precisa do mesmo mecanismo de limite de crédito que o Associado já tinha ganhado no mesmo dia.

**O que mudou:** `SolicitacaoCredito.associadoId` virou opcional, novo campo `agenciaId` opcional + relação com `Agencia` — mutuamente exclusivos, garantido por CHECK no banco (`solicitacao_credito_dono_unico`, mesmo padrão de `cobranca_valor_definido`). `credito.service.ts` generalizado pra aceitar `{entityType: 'associado'|'agencia', entityId}` em `solicitarCredito`/`listarMeusCreditos`/`atualizarCredito`/`deletarCredito`; `finalizarCredito` ganhou branch pra incrementar `Agencia.limiteCredito` + `Conta.limiteCredito` (mesma lógica de sincronização já usada pro Associado, saldo nunca é tocado). `listarCreditosMatriz` passou a incluir pedidos `em_analise` de Agência direto na fila da Matriz (mesma regra já existente pra associado sem agência — ninguém acima pra encaminhar). `encaminharCredito` ganhou guard defensivo — pedido da própria Agência não pode ser encaminhado (não existe encaminhamento nesse caso). Rotas `POST/GET/PUT/DELETE /creditos*` (self-service) passaram a aceitar `agency_admin`/`agency_operator` além de `associate_admin`/`associate_operator`. Front: `constantCreditos.js`, `CreditosModal.jsx`, `CreditoSolicitar.jsx` generalizados pra mostrar `associado?.nome ?? agencia?.nome` (renomeado "Associado"→"Solicitante" onde aplicável); `souDono`/`podeEncaminhar` no modal agora cobrem os dois tipos de dono.

**Validado:** `npx tsc --noEmit` limpo; `npm test` 32/32; `npm run build` sem erro; contra API/Postgres reais em Docker — criada Agência de teste, solicitado aumento de RT$5.000 como `agency_admin`, confirmado aparecendo direto na fila `em_analise` da Matriz (sem etapa de encaminhamento); aprovado via `PATCH /creditos/:id/aprovar`, confirmado `Agencia.limiteCredito` e `Conta.limiteCredito` 0→5.000 em sincronia, `Conta.saldo` inalterado; CHECK constraint testado diretamente via SQL — INSERT com nenhum dono e com os dois donos preenchidos, ambos rejeitados como esperado. Dados de teste removidos ao final.

## [RESOLVIDO 2026-08-26] Ofertas mostrava a própria oferta do associado + estoque não atualizava após compra
Dois achados do usuário, mesma tela ("Ofertas"):

**1. `GET /ofertas` mostrava a própria oferta de quem estava logado** — diferente da tela "Associados" (que já exclui o próprio associado da listagem, ver entrada `[RESOLVIDO]` de listDiretorio nesta mesma seção), a listagem pública de ofertas não excluía a oferta do requisitante. Rota é pública (sem `authGuard`) — precisava saber quem é o requisitante SEM exigir login.

**O que mudou:** novo `optionalAuthGuard` (`auth.guard.ts`) — decodifica o Bearer token se presente, mas nunca rejeita a requisição (segue anônimo se ausente/inválido). `GET /ofertas` ganhou esse preHandler; `offer.service.ts::list()` ganhou parâmetro `exceptContaId` que filtra `contaId: {not: exceptContaId}` quando o requisitante está logado.

**2. Card "Disponível" na tela de detalhe da oferta nunca diminuía após comprar** — `OfertasInfo.jsx` lia a oferta inteira de um snapshot no `localStorage` (`ofertaCard`), setado uma única vez no clique do card em `Ofertas.jsx`, e nunca re-buscava — mesmo com o backend decrementando `quantidadeDisponivel` corretamente a cada compra (`transaction.service.ts:174`, confirmado por teste direto via API: 5→3 após comprar 2). Um bug puramente de front-end, mesma classe dos vários "snapshot que nunca atualiza" corrigidos nesta sessão.

**O que mudou:** `OfertasInfo.jsx` reescrito pra buscar a oferta viva via `useQuery(['oferta', id], ...)` (só usa o `localStorage` pra saber QUAL id mostrar). Após `POST /transacoes/permuta` bem-sucedido, invalida `['oferta', id]` e `['ofertas']` via `queryClient.invalidateQueries` (sintaxe correta do React Query v5 — objeto `{queryKey}`, não string posicional).

**Nota (não corrigido agora):** `useRevalidate.js` (usado em ~30 lugares) chama `invalidateQueries(stringPosicional)` — sintaxe da v4, que na v5 não filtra por queryKey nenhum e acaba invalidando TODAS as queries ativas (funciona, mas de forma cara/imprecisa). Não mexido agora por ser fora do escopo pedido e writing risco de regressão em 30 arquivos; considerar padronizar pra `{queryKey: [...]}` numa rodada dedicada.

**Validado:** `npx tsc --noEmit` limpo; `npm test` 32/32; `npm run build` sem erro; contra API/Postgres reais em Docker — testado `GET /ofertas` como vendedor (oferta própria ausente), como comprador (presente) e anônimo (presente); comprado via API direta confirmado 5→3; Playwright na UI real confirmado "Disponível: 3" → "Disponível: 2" na mesma tela, sem reload, após comprar pelo formulário. Dados de teste removidos ao final.

## [RESOLVIDO 2026-08-26] "Vouchers" e "Meus Vouchers" sempre vazios (400 silencioso)
`useQueryVoucher.js`/`useQueryVoucherUsuario.js` chamavam `GET /transacoes?page=1&limit=200`, mas `listTransactionQuerySchema` (`transaction.schema.ts`) limita `limit` a `.max(100)` — toda chamada retornava `400 VALIDATION_ERROR`, e como `data?.data ?? []` engolia o erro silenciosamente, as duas telas ("Vouchers" e "Meus Vouchers") sempre mostravam a lista vazia, pra qualquer usuário, sem nenhum indício visual do motivo. Achado durante auditoria pedida pelo usuário ("verifique a lógica do voucher").

**O que mudou:** `limit=200` → `limit=100` nos dois hooks.

**Validado:** confirmado via Playwright na UI real — criada transação `negociada` (fluxo "Solicitar Voucher") entre dois associados de teste, "Meus Vouchers" antes da correção mostrava 0 linhas (rede confirmando 400), depois da correção mostrou as 4 transações esperadas com Comprador/Vendedor corretos.

**Achados adicionais, NÃO corrigidos (fora do escopo desta correção pontual, registrados pra decisão do usuário):**
1. O módulo `voucher.service.ts`/`voucher.controller.ts`/`voucher.routes.ts` (entidade `Voucher` de verdade — `codigo`, `GET /vouchers/:id`, `/pdf`, `/verificar/:codigo` público) **nunca é chamado pelo frontend**. `TransaçõesModal.jsx` recebe uma prop `voucher` de `Vouchers.jsx`/`MeusVouchers.jsx`/`CancelarVouchers.jsx` mas nunca a lê — o modal mostra os mesmos campos de qualquer transação comum (Vendedor/Comprador/Status/Descrição/Valor/Parcelas/Tipo), sem nenhum código de voucher, QR ou link de PDF em lugar nenhum do app.
2. `voucher.service.ts::getPdf()` não gera PDF de verdade — retorna um JSON (`{id, codigo, emitidoEm, transacao, pdfUrl}`) com comentário `// PDF generation would be done here`. Stub nunca finalizado.
3. `GET /vouchers/:id`/`/pdf` só aceitam `associate_admin`/`associate_operator` — Agência/Matriz (que também podem ser vendedoras via negociada) nunca conseguiriam acessar mesmo se o front chamasse. Sem checagem de dono também (qualquer associado autenticado poderia ver qualquer voucher por id, não só os próprios) — mas como o endpoint é morto, sem risco prático hoje.
4. `PDFVoucher.jsx` (rota órfã `/pdfVoucher`, nada no app navega até ela) é código legado quebrado — lê `state.user.usuario.dadosGerais.nomeFantasia`/`state.user.usuario.conta` (formato que não existe mais na API atual), tem `RT$ 800,00` hardcoded e um `''` solto no JSX (texto órfão depois da tag `<img>`).
5. A tela "Vouchers" (não-"Meus") é visível pra Agência/Matriz no menu, mas usa o mesmo `GET /transacoes` escopado pela própria conta do requisitante (`contaOrigemId`/`contaDestinoId`) — então Agência/Matriz só veem as próprias transações como comprador/vendedor, nunca uma visão consolidada dos vouchers dos seus associados/agências, apesar do nome da tela sugerir isso. O `// TODO: API precisa de GET /vouchers` já deixado no código por sessão anterior aponta exatamente essa lacuna — precisaria de um endpoint dedicado com escopo hierárquico (Agência vê dos seus associados, Matriz vê tudo), não corrigido agora por ser feature nova, não bug.
6. Toda transação `permuta`/`negociada` (independente de `tipoAtendimento` do vendedor) gera um `Voucher` automaticamente (1:1, `transaction.service.ts:177/337`) — ou seja, "voucher" hoje é sinônimo de "recibo de qualquer transação", não exclusivo de vendedores com `tipoAtendimento: voucher`. Pode ser intencional (todo negócio gera comprovante) ou pode ser a causa raiz de #5/#1 nunca terem sido finalizados — vale confirmar a intenção de produto antes de decidir se filtra ou não.

## [RESOLVIDO 2026-08-26] Voucher: entidade real nunca era exibida (código, comprovante, verificação pública)
Continuação da auditoria de voucher desta sessão — itens 1-4 e 6 dos achados anteriores:

**O que mudou:**
- `voucher.service.ts`: `getById`/`getPdf`/`verificar` agora incluem `contaOrigem`/`contaDestino` (resolve Agência/Matriz como comprador/vendedor, mesmo padrão do resto do sistema). Nova checagem de dono (`podeVer`) — Associado só vê voucher de transação onde participou; Agência vê o próprio ou dos seus associados; `superadmin` vê qualquer um; quem não tem acesso recebe `404`.
- `voucher.routes.ts`: guard de `GET /vouchers/:id`/`/pdf` estava restrito a `associate_admin/operator` (Agência/Matriz nunca conseguiam acessar, mesmo sendo parte na transação) — agora aceita também `agency_admin/operator` e `superadmin`, com a checagem de dono acima cobrindo a autorização fina.
- `TransaçõesModal.jsx` passou a ler a prop `voucher` (recebida por `Vouchers.jsx`/`MeusVouchers.jsx`/`CancelarVouchers.jsx` mas nunca lida antes) — quando a transação tem voucher, mostra o código e dois botões: "Baixar comprovante" (busca `GET /vouchers/:id` autenticado e gera PDF no cliente) e "Verificar publicamente" (abre `/voucherVerificar/:codigo` em nova aba).
- Novo `exportVoucherPdf.js` — gera o comprovante em PDF via `jsPDF` (mesmo padrão já usado por `exportContasPdf.js`/`exportEstornosPdf.js`, sem depender de geração de PDF binário no servidor).
- Nova tela pública `/voucherVerificar/:codigo` (`VoucherVerificar.jsx`) — consulta `GET /vouchers/verificar/:codigo` sem login, mostra os dados e permite baixar o mesmo comprovante.
- Removido `PDFVoucher.jsx`/`pdf.css` (código legado morto por trás da rota órfã `/pdfVoucher` — ninguém no app navegava até lá; lia campos que não existem mais na API atual, tinha `RT$ 800,00` hardcoded e um `''` solto no JSX) — substituído pela rota `/voucherVerificar/:codigo`.

**Mantido como estava (decisão consciente, não é bug):** todo `permuta`/`negociada` continua gerando voucher automaticamente, independente do vendedor aceitar `tipoAtendimento: voucher` — mudar isso é decisão de negócio maior, fora do escopo desta correção.

**Ainda fora do escopo (feature nova, não bug — registrado, não implementado):** a tela "Vouchers" (não-"Meus", visível pra Agência/Matriz) continua usando `GET /transacoes` escopado pela própria conta do requisitante — não existe ainda uma visão hierárquica consolidada (Agência vendo vouchers de todos os seus associados, Matriz vendo tudo). Precisaria de um endpoint dedicado, mesmo padrão de `listarCreditosFilhos`/`listarCreditosMatriz` do módulo de Crédito.

**Validado:** `npx tsc --noEmit` limpo; `npm test` 32/32 (backend); `npm run build` sem erro (front); contra API/Postgres reais em Docker + Playwright — criada transação voucher entre dois associados de teste, confirmado: modal mostra o código, "Baixar comprovante" dispara download real (`voucher-<codigo>.pdf`), "Verificar publicamente" abre a página pública com os dados corretos. Testado o guard de dono via API direta: vendedor (participou) recebe `200`; terceiro associado não-participante recebe `404`; verificação pública por código funciona sem token. Dados de teste removidos ao final.

## [RESOLVIDO 2026-08-26] Tela "Vouchers" (Agência/Matriz) sem visão consolidada
Último item pendente da auditoria de voucher desta sessão (item 5): a tela "Vouchers" (visível pra Agência/Matriz, distinta de "Meus Vouchers") usava o mesmo `GET /transacoes` escopado pela própria conta do requisitante — Agência só via as transações onde ela mesma foi parte, nunca os vouchers dos seus associados, apesar do nome sugerir uma visão administrativa.

**O que mudou:** novo endpoint `GET /vouchers` (`voucher.service.ts::listar()`), mesmo padrão hierárquico já usado em `listarCreditosFilhos`/`listarCreditosMatriz` do módulo de Crédito — Agência vê vouchers de transações onde ela mesma participou OU onde um associado dela participou (`transacao.comprador/vendedor.agenciaId` OU `transacao.contaOrigem/contaDestino.agenciaId`); Matriz (`superadmin`) vê tudo, sem filtro. `Vouchers.jsx` migrado do hook antigo (`useQueryVoucher`, `/transacoes`) pro novo (`useQueryVouchersConsolidado`, `/vouchers`) — `MeusVouchers.jsx`/`VoucherSolicitarCancelar.jsx` continuam em `/transacoes` (correto pra visão "minhas transações"). `TransaçõesModal.jsx` ganhou resolução dupla de `voucherData` (`GET /vouchers` retorna Voucher com `.transacao` aninhada — tem `.codigo` no topo; `GET /transacoes` retorna Transacao com `.voucher` aninhado) pra funcionar em ambas as telas.

**Validado:** `npx tsc --noEmit` limpo; `npm test` 32/32; `npm run build` sem erro; contra API/Postgres reais em Docker + Playwright — criada Agência com um associado vendedor, uma transação com comprador externo, e uma segunda Agência sem nenhuma relação: `GET /vouchers` como a Agência dona retornou 1 (a transação do associado dela), como a Agência não relacionada retornou 0, como Matriz retornou todas (incluindo essa). Confirmado na UI real: Agência logada vê a linha correta em "/voucher", modal mostra código do voucher e os botões de comprovante/verificação. Dados de teste removidos ao final.

## [RESOLVIDO 2026-08-26] Validação: valor de inscrição em RT não pode passar do limite de crédito
Sugestão do usuário ao ver que o cadastro de Associado deixava configurar `limiteCredito` menor que `valorInscricaoRT` (ex: limite 0, inscrição RT$ 1.000) — a regra de saldo nunca era violada de fato (a quitação da cobrança já revalida saldo/limite corretamente e bloqueia com `INSUFFICIENT_BALANCE`), mas a cobrança de inscrição ficava travada pra sempre, sem ninguém conseguir quitar.

**O que mudou:** `createAssociateSchema` (`associate.schema.ts`) ganhou `.refine()` — rejeita o cadastro com `VALIDATION_ERROR` se `valorInscricaoRT > limiteCredito`. Mesma checagem espelhada no front (`associadoSchema.js`, `.refine()` com os valores parseados da máscara de moeda) pra feedback inline antes mesmo de submeter.

**Escopo:** só na criação (`POST /associados`), que é onde a cobrança de inscrição é gerada. Não se aplica a `PUT /associados/:id` — edição posterior de `limiteCredito` já é protegida pela revalidação de saldo/limite em tempo real que `cobranca.service.ts::quitarCobranca()` sempre faz no momento de quitar (não depende do valor histórico de `valorInscricaoRT`).

**Achado relacionado, não corrigido (fora do que foi pedido):** o formulário de cadastro de **Agência** tem campos visualmente idênticos ("Valor em Dinheiro (R$)"/"Valor em Permuta (RT$)"), mas `CadastrarAgencia.jsx` não os lê nem envia — não existe cobrança de inscrição pra Agência hoje, então esses campos são decorativos. Diferente do problema do Associado (cobrança travada), aqui o comportamento é "não faz nada". Registrado pra decisão futura, não implementado.

**Validado:** `npx tsc --noEmit` limpo; `npm test` 32/32; `npm run build` sem erro; contra API real: `POST /associados` com `limiteCredito: 0, valorInscricaoRT: 1000` rejeitado com a mensagem esperada; com `limiteCredito: 1000, valorInscricaoRT: 1000` (igual) aceito normalmente. Schema do front testado isoladamente (`safeParse`) com os mesmos casos, mesmo resultado.

## [RESOLVIDO 2026-08-26] Edição de Associado não mostrava como/quanto foi pago na inscrição
Pedido do usuário: documentar no modal de edição do Associado quanto e como a inscrição foi paga (dinheiro/permuta), sem poder editar depois — esses valores já viram uma `Cobranca` no cadastro, editar os campos históricos depois não faz sentido (é a Cobrança que rege o que está em aberto).

**O que mudou:** `EditarAssociadoModal.jsx` ganhou 3 campos `readOnly` (sem atributo `name`, então nunca vão no `PUT /associados/:id` mesmo se alguém adulterar o DOM): "Forma de Pagamento do Plano" (rótulo do código salvo — `0`=Dinheiro, `50`=Permuta/Dinheiro, `100`=Permuta, mesmo mapa já usado no dropdown do cadastro), "Valor em Dinheiro (R$)" e "Valor em Permuta (RT$)" — mostram `associado.valorInscricaoBRL`/`valorInscricaoRT` exatamente como foram digitados no cadastro, sem nenhum cálculo (nenhuma divisão automática entre os dois valores).
...
## [Decisão de produto 2026-08-27] Cobrança de inscrição em RT já nasce quitada e debitada
O usuário testou em produção (não no ambiente local) e viu o dashboard do associado recém-cadastrado com `Saldo em Permutas: RT$ 0,00`, esperando `RT$ -2.000,00` (o valor de inscrição em RT configurado). Reproduzi localmente pra confirmar antes de mexer: cadastrar associado com `limiteCredito: 2000`/`valorInscricaoRT: 2000` deixava `saldo = 0.00` e criava uma `Cobranca` (`tipo: 'inscricao'`, `pago: false`) — só ficava `-2000.00` depois de alguém ir na tela de Cobranças e "Quitar" manualmente. Decisão explícita do usuário: inscrição em RT deve debitar na hora do cadastro, mas continuar registrada como cobrança (auditoria).

**O que mudou:** `associate.service.ts::create()` — o bloco que criava a `Cobranca` de inscrição em RT (pendente) agora, na mesma transação: cria a `Cobranca` já com `pago: true`, debita `valorInscricaoRT` da conta do associado recém-criado, credita a conta da Agência (`input.agenciaId`) ou da Matriz (associado direto), com as duas pernas de `MovimentacaoConta` (mesma mecânica de `cobranca.service.ts::quitarCobranca()`, só que disparada automaticamente em vez de esperar uma ação manual). A cobrança em **BRL** não foi tocada — continua pendente até reconciliação manual (é liquidada fora do sistema, via PIX/boleto; não tem como o sistema saber sozinho se foi paga).

**Por que é seguro:** `limiteCredito >= valorInscricaoRT` já é garantido pelo `.refine()` do Zod adicionado na sessão anterior (mesmo dia) — o associado sempre nasce com `saldo = 0`, então o débito nunca pode violar o CHECK `saldo >= -limiteCredito` do banco.

**Validado:** `npx tsc --noEmit` limpo; `npm test` 32/32; contra API/Postgres reais em Docker — cadastrado associado com limite/inscrição RT$2.000 (sem agência): `saldo` confirmado `-2000.00` imediatamente após o cadastro (sem precisar quitar nada), `Cobranca.pago = true`, saldo da Matriz subiu exatamente RT$2.000 (dupla partida íntegra); repetido com um associado vinculado a uma Agência: saldo da Agência subiu RT$300 (não a Matriz), confirmando que a contraparte é resolvida corretamente. Dados de teste e saldos de Matriz/Agência restaurados ao estado original ao final.

## [RESOLVIDO 2026-09-08] Card "Ofertas" do dashboard sempre mostrava Unidade 0
Regressão introduzida sem querer dentro da própria sessão anterior: `49d0675` ("Ofertas exclui a própria e estoque atualiza após compra") mudou `GET /ofertas` pra excluir a oferta do próprio requisitante — correto pra tela de marketplace, mas `OfertasCard_Dashboard.jsx` reaproveitava a mesma `useQueryOfertas()` pra calcular "Unidade" (minhas ofertas) e "Geral" (todas) — com a exclusão, "Unidade" sempre dava 0 (a própria oferta nunca vinha na lista pra ser filtrada) e "Geral" ficava subcontado por 1 pra cada conta que tivesse alguma oferta ativa. Achado ao validar o pedido do usuário de revisar a regra Unidade/Geral do dashboard depois da limpeza do ambiente online.

**O que mudou:** `OfertasCard_Dashboard.jsx` passou a somar de volta via `GET /ofertas/minha-loja` (nunca exclui, mas inclui `fechada`/`pausada` — filtrado só pras `aberta`, pra bater com o critério do marketplace). `Unidade = minhasAbertas.length`, `Geral = outras.length + minhasAbertas.length`.

**Validado:** `npx eslint`/`npm run build` sem erro; contra API/Postgres reais (rodei a API local numa porta alternativa, já que a porta 3000 padrão estava ocupada por outro projeto na máquina) — criada Agência com um associado (A2) e um associado direto da Matriz (A1), cada um com uma oferta própria: confirmado `GET /ofertas` como A1 retorna só a oferta de A2 (exclusão funcionando), `GET /ofertas/minha-loja` como A1 retorna só a oferta de A1; aplicando a fórmula corrigida bate exatamente Unidade=1/Geral=2 (o total real do sistema). Card "Associados" (mesma regra Unidade/Geral) revalidado também: Matriz com 1 associado direto + 1 sob uma Agência confirmou Unidade=1/Geral=2, sem regressão. Dados de teste removidos ao final.

## [RESOLVIDO 2026-09-08] Card "Associados" do dashboard sempre zerado pro Associado comum
Achado do usuário — logou como um Associado comum ("Click Dogs", cadastrado direto pela Matriz) e o card "Associados" mostrou `Unidade 0, Geral 0`, enquanto "Ofertas" no mesmo dashboard mostrou valores reais (`1/1`). Causa: `AssociadoCard_Dashboard.jsx` deliberadamente zerava os dois campos pra qualquer Associado que não fosse Agência/Gerente — decisão de uma correção anterior desta sessão, que assumiu que um associado comum não tinha nenhuma fonte de dados acessível pra essa contagem. Não é o caso — `GET /associados/diretorio` já é acessível pra esse role.

**Regra confirmada com o usuário:** associado cadastrado direto pela Matriz (sem agência) → Unidade e Geral mostram o mesmo valor (o total do sistema). Associado vinculado a uma Agência → Unidade mostra o total de associados **daquela agência**, Geral continua sendo o total do sistema (o que "fica com a Matriz").

**O que mudou:**
- `auth.service.ts::me()` — `GET /auth/me` passou a retornar `agenciaId` (do `Usuario` logado) — não vinha antes, e é a única forma de saber a que agência um Associado pertence sem duplicar a query do diretório.
- `AssociadoCard_Dashboard.jsx` — Associado comum (não Agência, não Gerente) agora usa `GET /associados/diretorio` pra calcular Unidade/Geral. Esse endpoint **exclui o próprio requisitante** (correto pro seletor de parceiro de negociação na tela Associados, onde foi originalmente pensado) — compensado somando `+1` de volta nas duas contagens. Sem `agenciaId` (direto da Matriz), Unidade = Geral; com `agenciaId`, Unidade filtra só quem tem a mesma agência.
- `useQueryAssociadosDiretorio.js` ganhou parâmetro `enabled` (default `true`, não quebra os 4 outros usos existentes) — evita chamar a rota à toa pra Matriz/Agência, que usam outra fonte de dados nesse mesmo card.

**Validado:** `npx tsc --noEmit` limpo; `npm test` 32/32; `npx eslint`/`npm run build` sem erro; contra API/Postgres reais (API local numa porta alternativa) — reproduzido o cenário exato do usuário: 1 associado direto da Matriz, logado como ele, confirmado `GET /auth/me.agenciaId = null` e cálculo resultando em `Unidade=1, Geral=1` (antes: `0/0`). Adicionada uma Agência com 2 associados: logado como um deles, confirmado `agenciaId` preenchido e cálculo `Unidade=2, Geral=3` (2 da mesma agência, 3 no total do sistema). Dados de teste removidos ao final.

## [RESOLVIDO 2026-09-08] Card "Ofertas" — "Unidade" não incluía ofertas de associados diretos da Matriz
Segunda rodada de correção do mesmo card no mesmo dia — a correção anterior (`589db0a`) tratou "Unidade" como "só a minha própria conta", mas o usuário testou em produção com um associado cadastrado direto pela Matriz criando uma oferta, e ela não aparecia em "Unidade" da Matriz (só em "Geral"). A regra correta, confirmada com o usuário, é a mesma dos outros 3 cards (Associados, Permutas Mês, Fundo Permuta): "Unidade" = eu + todo mundo do meu grupo hierárquico (Matriz: associados sem agência; Agência: seus próprios associados), não só minha própria conta.

**O que mudou:**
- `offer.service.ts::list()` — include de `conta.associado` ganhou `agenciaId` (só tinha `nome`) — necessário pro front saber a que grupo cada oferta pertence.
- `OfertasCard_Dashboard.jsx` — nova função `pertenceAoMeuGrupo(oferta)` replicando a mesma regra hierárquica do `AssociadoCard_Dashboard.jsx`: Matriz conta ofertas de associados sem `agenciaId`; Agência conta ofertas de associados com `agenciaId === meuEntityId`; Associado comum conta ofertas de colegas da mesma agência (ou colegas diretos da Matriz, se ele também não tem agência). "Unidade" agora é `minhasAbertas + outras.filter(pertenceAoMeuGrupo)`.

**Validado:** `npx tsc --noEmit` limpo; `npm test` 32/32; `npx eslint`/`npm run build` sem erro; contra API/Postgres reais (API local numa porta alternativa) — reproduzido o cenário exato do usuário: associado direto da Matriz cria uma oferta, logado como Matriz confirma `Unidade=1, Geral=1`; adicionada uma Agência com um associado dela criando outra oferta, confirmado como Matriz `Unidade=1 (só a do direto), Geral=2`, e como a Agência `Unidade=1 (só a do associado dela), Geral=2` — a hierarquia isola corretamente cada grupo. Dados de teste removidos ao final.

## [RESOLVIDO 2026-09-08] Cards "Permutas Mês" e "Fundo Permuta" não contabilizavam o grupo hierárquico
Terceira e quarta ocorrência do mesmo padrão de bug nesse mesmo dia (depois de Associados e Ofertas): ambos os cards calculavam "Unidade" olhando só a própria conta/registros administrativos, sem aplicar a regra "Unidade = eu + meu grupo hierárquico" já estabelecida pros outros 2 cards. Achado ao pedido do usuário pra analisar os dois cards depois de confirmar a correção de Ofertas.

**Permutas Mês:** `useQueryPermutasMes.js` calculava "Unidade" via `GET /transacoes` (escopado só pela própria conta) — uma transação entre dois associados diretos da Matriz não aparecia na "Unidade" dela, só na "Geral". Testado e confirmado o bug antes de corrigir (transação de RT$500 entre 2 associados diretos, Matriz via Unidade=0/Geral=500).

**Fundo Permuta:** usava `GET /associados`/`GET /agencias`/`GET /agencias/:id/associados` (endpoints administrativos) pra somar `limiteCredito` — todos bloqueados (403) pra um Associado comum, que ficava sempre com Unidade=0 mesmo tendo seu próprio `limiteCredito`. `limiteCredito` é dado financeiro, não podia simplesmente reaproveitar o diretório público (que deliberadamente omite esse campo) como fiz pro card Associados — precisou de endpoint novo que soma no banco sem expor valores individuais.

**O que mudou:**
- Novo `GET /relatorios/permutas-mes` (`resumoPermutasMes()`) — calcula `unidade`/`geral` via `aggregate` no banco, com a mesma regra hierárquica de `contaOrigem`/`contaDestino` já usada nos outros relatórios (Matriz: ela mesma + associados sem agência; Agência: ela mesma + seus associados; Associado: colegas da mesma agência ou diretos da Matriz).
- Novo `GET /relatorios/fundo-permuta` (`resumoFundoPermuta()`) — mesma regra, somando `limiteCredito` de `Agencia`/`Associado` via `aggregate`, sem retornar nenhum valor individual — seguro pra qualquer role, incluindo Associado comum.
- Ambos endpoints abertos pra qualquer role autenticada (incluindo `gerente`, que não tinha acesso a nada disso antes).
- `useQueryPermutasMes.js` e `FundoPermutaCard_Dashboard.jsx` simplificados pra só consumir os novos endpoints — toda a lógica de agrupamento saiu do front.

**Validado:** `npx tsc --noEmit` limpo; `npm test` 32/32; `npx eslint`/`npm run build` sem erro; contra API/Postgres reais (API local numa porta alternativa) — cenário com 1 associado direto da Matriz + 1 Agência com 1 associado dela, valores de teste escolhidos pra não coincidir por acaso: Permutas Mês confirmado Matriz `Unidade=500/Geral=700`, Agência e seu associado `Unidade=200/Geral=700`; Fundo Permuta confirmado Matriz `Unidade=4500/Geral=5200`, Agência e seu associado `Unidade=700/Geral=5200`. Dados de teste removidos ao final.

## [Decisão de produto 2026-09-08] Cadastro de Associado: Limite Crédito automático + preenchimento assistido de Dinheiro/Permuta
Pedido do usuário: já que a inscrição em RT debita a conta na hora do cadastro (decisão anterior, mesmo dia), o campo "Limite Crédito" deveria acompanhar automaticamente o "Valor em Permuta" — sem risco de configurar um limite menor que o débito que vai acontecer. Também pediu preenchimento assistido: ao digitar Dinheiro ou Permuta (forma de pagamento "Permuta/Dinheiro"), o outro campo já calcula o restante do Valor do Plano, pra nunca a soma passar nem ficar abaixo do total.

**O que mudou:**
- `FormInputMoney.jsx` ganhou prop opcional `onValueChange(numero)` — dispara com o valor numérico digitado, sem quebrar os outros usos (prop opcional).
- `Form_Agencia.jsx` (seção "Agência" do cadastro de Associado): forma de pagamento "Dinheiro" ou "Permuta" (100% de um jeito só) já preenche o campo único com o valor total do plano e trava ele (`disabled`) — não tem outra divisão válida. Forma "Permuta/Dinheiro" (misto): digitar um dos dois calcula o outro como `valorPlano - valorDigitado` (nunca negativo), os dois continuam editáveis.
- `Form_Operacoes.jsx`: campo "Limite Crédito" trocou de editável pra `disabled`, e segue automaticamente `valorInscricaoRT` via `useWatch`+`useEffect` — nunca mais digitado manualmente nesse formulário.

**Validado:** `npx eslint`/`npm run build` sem erro; contra API/Postgres reais em Docker + Playwright — plano de teste com Valor do Plano RT$1.500,00: selecionado "Permuta/Dinheiro" e digitado R$750,00 em Dinheiro, confirmado "Valor em Permuta" calculado automaticamente pra RT$750,00 e "Limite Crédito" também RT$750,00 (campo travado); trocado pra "Permuta" (100%), confirmado ambos os campos RT$1.500,00 (valor total do plano); cadastro completo enviado com sucesso e conferido no banco: `limiteCredito=750`, `valorInscricaoBRL=750`, `valorInscricaoRT=750`, `saldo=-750` (débito imediato dentro do limite, sem violar o CHECK). Dados de teste removidos ao final.

## [RESOLVIDO 2026-09-08] Cadastro de Associado: valor "fantasma" ao trocar de forma de pagamento
Achado do usuário ao testar a correção anterior no mesmo dia: alternar entre "Permuta" → "Dinheiro" → "Permuta/Dinheiro" deixava os dois campos com o valor cheio do plano (ex: RT$1.500,00 e R$1.500,00 juntos) em vez de dividir — porque o `useEffect` só atualizava o campo do modo que estava entrando, nunca limpava o campo que tinha ficado escondido no modo anterior (o valor "sobrava" de quando aquele campo esteve ativo). Faltava também um ponto de partida sensato ao entrar no modo misto — o usuário pediu 50/50 por padrão, em vez de zerado.

**O que mudou:** `Form_Agencia.jsx` — o `useEffect` que reage a `formaPagamento`/`planoValor` agora recalcula **os dois campos inteiros** em toda troca de modo, nunca só o visível: "Dinheiro" preenche BRL com o valor total e **zera** RT; "Permuta" preenche RT com o valor total e **zera** BRL; "Permuta/Dinheiro" preenche os dois com metade do valor do plano cada (ponto de partida, ajustável digitando — mantém o cálculo do restante já existente).

**Validado:** `npx eslint`/`npm run build` sem erro; contra API/Postgres reais em Docker + Playwright — reproduzido o cenário exato do usuário (Permuta → Dinheiro → Permuta/Dinheiro), confirmado 750,00/750,00 nos dois campos (não mais 1.500/1.500); testado também o caminho direto (selecionar misto sem passar por outro modo antes) e digitar primeiro o campo Permuta em vez do Dinheiro — ambos corretos; voltar de misto pra um modo puro confirmado sem sobra (R$1.500,00 limpo, sem resquício da divisão anterior).

## [RESOLVIDO 2026-09-08] Tela "Transações" (Matriz/Agência): filtros obrigatórios travando a busca + fonte de dados errada
Achado do usuário: campos "Associado" e "Agência" tinham `required` no HTML — sem nenhuma agência cadastrada, literalmente não dava pra clicar em "Pesquisar" (navegador bloqueia). Investigando a fundo, achei um problema mais estrutural por trás: a tela "Transações" (`/transacoes`, distinta de "Minhas Transações") usava `GET /transacoes`, escopado só pela própria conta do requisitante — como Matriz, "olhar todas as transações" nunca funcionou de verdade, independente do filtro, porque a fonte de dados só trazia as transações em que a própria Matriz participou.

**O que já existia (achado, não construído agora):** uma versão correta desse mesmo padrão já tinha sido implementada nesta sessão pra tela "Extratos" — `useQueryRelatorioTransacoes` (`GET /relatorios/permutas`, visão hierárquica: Matriz vê tudo, Agência vê a própria conta + associados) e `estratos/constantsTransacoes.js` (colunas com filtro de verdade via `filterIncludes`/`filterIncludesId`, sem o campo legado `conta.nomeFranquia`). A tela "Transações" nunca foi migrada pra essa infraestrutura já validada.

**O que mudou:**
- `report.service.ts::relatorioPermutas()` e a rota `/relatorios/permutas` passaram a aceitar também `agency_operator` (antes só `agency_admin`/`superadmin`/`associate_admin` — mas `agency_operator` também vê a tela "Transações" no menu).
- `SearchfieldTrade.jsx` reescrito seguindo o padrão já corrigido de `ExtratosSearch.jsx`: sem `required`, "Associado"/"Agência" com "Todos"/"Todas" como default (não bloqueia a busca), `onSubmit` com `preventDefault` (sem mais recarregar a página), "Associado" agora usa `GET /associados/diretorio` (broadly acessível, `value=item.id`) em vez do endpoint admin-only com `value=nomeFantasia` quebrado.
- `Transações.jsx` migrado de `useQueryTransacoes`/`./constants.js` (self-scoped, coluna Agência quebrada) pra `useQueryRelatorioTransacoes`/`estratos/constantsTransacoes.js` (visão hierárquica, filtros funcionais).
...
## [RESOLVIDO 2026-09-09] Texto sumindo em todo `<select>` do sistema (fica branco ao passar o mouse)
Achado do usuário: em qualquer dropdown do sistema, passar o mouse sobre uma opção (ou a opção já selecionada) deixava o texto ilegível/branco. Causa: `index.css` é o **boilerplate padrão do Vite** pra um projeto React novo, nunca customizado pra esse app — tinha `color-scheme: light dark` no `:root`, dizendo ao navegador que a página suporta os dois temas. O app inteiro é desenhado só pra tema claro (fundo branco, texto escuro em toda parte), mas essa declaração faz o navegador renderizar **controles nativos** (lista de opções do `<select>`, scrollbar) seguindo a preferência de tema escuro do sistema operacional do usuário — só que sem nenhum CSS do app cobrindo esse caso, o resultado é uma combinação de cores ilegível. Acontecia em todo `<select>` porque é uma declaração global no `:root`.

Já existia um precedente exatamente pra esse tipo de problema em `App.css:1895` (`color-scheme: light` num input específico) — só nunca tinha sido aplicado no `:root` global de `index.css`, onde o vazamento realmente acontecia.
...
## [Decisão técnica 2026-09-11] Índice único parcial da comissão mensal sem CONCURRENTLY
O `db-migration-checker` apontou que `CREATE UNIQUE INDEX` (migration `20260911201308_cobranca_competencia_comissao`) sem `CONCURRENTLY` toma lock de escrita na tabela `cobranca` durante a criação do índice. Decisão: manter sem `CONCURRENTLY`.
...
## [RESOLVIDO 2026-09-15] Dashboard "Resumo Administrativo": 3 campos ligados a formato de API que nunca existiu
Achado do usuário (screenshot): "A Receber Associados", "A Pagar Gerentes" e "A Receber Agência" sempre em RT$ 0,00 na Matriz. Investigação mostrou que é bem mais amplo que "só pra Matriz" — `ResumoFinanceiro.jsx` lia campos (`valorTotalReceber`, `aReceberRepasses.valorTotalReceberMatriz`, `aReceberCobrancas.valorTotalCobrancas`) que **nenhum endpoint do backend jamais retornou**, pra nenhum papel — confirmado via `grep` no backend inteiro, zero ocorrências desses nomes. Também achados dois blocos JSX mortos (comparando `getType() === "Associado - faill"` / `"Franquia Comum"` / `"Franquia Master"`, strings que o próprio `getId.js` já documentava como impossíveis de bater pra quem não é superadmin) — removidos junto, já que referenciavam as mesmas variáveis quebradas.

**O que mudou:**
- **Sem endpoint novo** — o dashboard agora reaproveita a mesma fonte de dados da tela "Contas a Receber" (`useQueryContasReceberMatriz`/`useQueryContasReceber`, já existentes e já corretos), garantindo que dashboard e tela nunca divirjam. "A Receber Associados"/"A Receber Agência" são subtotais calculados no frontend a partir dessa mesma lista: `associadoId` preenchido = dívida de associado, só `agenciaId` (sem `associadoId`) = dívida direta da própria agência.
- **Cobrança pode ser BRL ou RT** (nunca as duas) — os totais somam as duas moedas separadas e mostram ambas quando aplicável (`R$ X + RT$ Y`), em vez de tratar tudo como uma moeda só (bug latente que a versão antiga já tinha).
- **Bug adicional encontrado e corrigido**: `relatorioComissoesGerentes` (`/relatorios/comissoes-gerentes`, alimenta "A Pagar Gerentes") somava só `comissaoBRL` — mas comissão de gerente **por transação** (a maioria dos casos, `commission.gerente` em `bullmq.ts`) é gravada em `comissaoRT`, não `comissaoBRL` (só comissão de *inscrição* usa BRL). O relatório ficava mostrando ~R$0 na prática mesmo com comissão real pendente. Agora soma as duas moedas separadas (`totalComissaoGerenteBRL` + `totalComissaoGerenteRT`).
- Removidos os hooks mortos `useQueryReceberAgencia.js`/`useQueryReceberAssociado.js` (chamavam `cobrancas/minhas`, nenhum outro consumidor).

**Validado:** `npx tsc --noEmit` e `npm test` (32/32) limpos; `npx eslint`/`npm run build` (front) sem erro; contra API/Postgres/Redis reais em Docker — criado gerente com plano de comissão 10%, associado vinculado a ele, 1 transação de 200 RT (comissão de plataforma R$16, comissão de gerente RT$20 gravada em `comissaoRT`); confirmado `GET /relatorios/comissoes-gerentes` retornando `totalComissaoGerenteRT: "20"` (antes ficaria em `totalComissaoGerenteBRL: "0"`, mascarando o valor real); job de consolidação mensal gerou a `Cobranca` de comissão da transação; `GET /cobrancas` confirmado com o subtotal exato que o frontend calcularia (R$16 em associados, R$0 em agência). Dados de teste removidos ao final.

## [RESOLVIDO 2026-09-15] Data de vencimento sempre 1 dia a menos do que o configurado
Achado do usuário: configurando o dia de vencimento da fatura pra 20, o sistema mostrava 19. Causa: o servidor roda em UTC (Docker), e `calcularVencimento()`/`diaClamped()` (`shared/utils/data.ts`) construíam a data com `new Date(ano, mes, dia)` — hora LOCAL do servidor, ou seja meia-noite UTC. Um navegador no fuso de Brasília (UTC-3) lê essa mesma data como 21h do dia ANTERIOR (`.getDate()` acaba mostrando dia-1). **Acontece com qualquer dia do mês, não só em meses de 31 dias** — é um deslocamento de fuso horário, não uma conta de calendário errada.

O mesmo tipo de bug JÁ tinha sido evitado corretamente em `inicioMesBrasilia()` (`shared/utils/limites.ts`, usado pelo job de consolidação de comissão) — só `calcularVencimento`/`diaClamped` (vencimento de inscrição, manutenção, comissão mensal — usado em quase toda cobrança do sistema) não seguiam esse mesmo padrão.

**O que mudou:** `data.ts` ganhou `meiaNoiteBrasilia(ano, mes, dia)`, que constrói a data já como 03:00 UTC (= meia-noite em Brasília), igual à técnica de `inicioMesBrasilia()`. `calcularVencimento()` e `diaClamped()` passaram a usar isso; `proximoVencimentoManutencao()` passou a ler `getUTCFullYear()/getUTCMonth()` em vez de `getFullYear()/getMonth()` (que dependiam do timezone do processo Node, não deveriam). Testes existentes (`data.test.ts`) reescritos pra esperar meia-noite de Brasília (antes travavam o comportamento com bug, porque tanto o código quanto o test runner rodam em UTC); adicionados 4 testes novos pra `calcularVencimento` (incluindo caso de borda perto da virada UTC/Brasília, com `vi.useFakeTimers`).

**Validado:** `npx tsc --noEmit` limpo; `npm test` 36/36 (32 anteriores + 4 novos); contra API/Postgres reais em Docker — `calcularVencimento(20)` retorna `2026-09-20T03:00:00.000Z`; simulação de leitura em fuso Brasília confirma dia 20 (antes dava 19); associado real criado com `diaVencimentoFatura: 20` gerou cobrança de inscrição com `vencimento: 2026-09-20 03:00:00` no banco. Dado de teste removido ao final.

## [Decisão de produto 2026-09-18] Comissão de gerente: regra de tipoOperacao, sempre BRL, pagamento mensal
Pedido do usuário, a partir da análise do bug "A Pagar Gerentes" (ver entrada anterior, 2026-09-15): `ComissaoGerente` era só INSERT sem status de pago, então o total nunca refletia o que ainda estava em aberto. Discutido em brainstorm (`/superpowers:brainstorming`) e fechado com as seguintes decisões:

**1. Quando a comissão é gerada — regra nova, usa `Associado.tipoOperacao` pela primeira vez (campo existia desde sempre mas nunca foi lido em lugar nenhum do código até agora).** Antes, `commission.gerente` só olhava o lado comprador (`contaOrigem`) — um gerente NUNCA recebia comissão quando o associado dele vendia. Agora avalia os dois lados (comprador e vendedor) independentemente:
- `tipoOperacao: 'compra'` → só comissiona no lado comprador, percentual cheio do gerente.
- `tipoOperacao: 'venda'` → só comissiona no lado vendedor, percentual cheio.
- `tipoOperacao: 'compra_venda'` → comissiona nos dois lados, mas com METADE do percentual em cada (10% → 5%+5%) — o gerente recebe o percentual cheio no total, não em dobro.
- Sem `tipoOperacao` configurado, ou `percentualComissao` do gerente em 0%, não gera nada.

**2. Comissão sempre em BRL, nunca RT.** Antes, comissão por transação calculava e gravava em `comissaoRT` (só comissão de inscrição usava `comissaoBRL`) — inconsistente com a comissão da plataforma, que já usa `valorRT × percentual/100 = reais` sem conversão. Unificado: mesma fórmula, resultado sempre em `comissaoBRL`. **Coluna `comissaoRT` removida do banco** (usuário confirmou explicitamente: sem dado histórico relevante, banco de staging seria zerado de qualquer forma).

**3. Pagamento consolidado mensalmente, marcado como pago sem mover saldo.** Novo modelo `PagamentoGerente` (gerenteId, competencia, valorBRL, pago, pagoEm) — deliberadamente SEPARADO de `Cobranca` (que representa dívida PRA Matriz, tem vencimento, pode ser RT, mexe em conta quando quitada em RT; pagamento de gerente é sempre BRL, sempre pago por fora do sistema — PIX/dinheiro — e "dar baixa" é só uma flag). Mesmo job mensal (`commission.consolidate`) que já consolidava a comissão da plataforma agora também roda `gerarPagamentosGerenteMensal()`, somando `ComissaoGerente.comissaoBRL` do mês fechado por gerente. Idempotente via `@@unique([gerenteId, competencia])`.

**4. Tabelas continuam separadas** (não unificadas num `comissao` genérico) — decisão explícita, ver troca de mensagens: comissão da plataforma (recebível, ciclo de vida de `Cobranca`) e comissão de gerente (pagável, ciclo de vida próprio) são conceitos estruturalmente diferentes; nome genérico "comissao" colidiria com `Cobranca.tipo = 'comissao'`, que já existe.

**O que mudou:**
- `manager.service.ts`: `registrarComissaoGerentePorTransacao()` (substitui a lógica inline do worker), `gerarPagamentosGerenteMensal()`, `listarPagamentosGerente()` (agency_admin só vê da própria agência), `quitarPagamentoGerente()`.
- `queues/bullmq.ts`: worker `commission.gerente` agora só delega pro service; `commission.consolidate` roda os dois jobs mensais (Cobranca + PagamentoGerente) juntos, mesmo mês de referência.
- Rotas novas: `GET /gerentes/pagamentos` (superadmin + agency_admin), `PATCH /gerentes/pagamentos/:id/quitar` (superadmin).
- `relatorioComissoesGerentes`/`/relatorios/comissoes-gerentes` simplificado de volta pra só `comissaoBRL` (era o relatório histórico bruto — continua existindo, mas não é mais a fonte do dashboard).
- Dashboard "A Pagar Gerentes" (`ResumoFinanceiro.jsx`) e tela "Comissões" (nova seção "A Pagar Gerentes", `PagamentoGerenteTable.jsx`) passam a usar a MESMA fonte de dados (`GET /gerentes/pagamentos`), somando só o que está `pago: false` — resolve o bug original (antes somava todo o histórico desde sempre).

**Validado:** `npx tsc --noEmit` e `npm test` (36/36) limpos; `npx eslint`/`npm run build` (front) sem erro; contra API/Postgres/Redis reais em Docker — criado gerente (10%), 3 associados (compra_venda, venda, só-compra), 2 transações: confirmado que o associado compra_venda gerou 5%+5% (dois lados), o associado venda gerou 10% completo, e o associado só-compra NÃO gerou comissão nenhuma ao vender (regra respeitada); consolidação mensal gerou 1 `PagamentoGerente` de R$35 (soma exata das 3 linhas); rodada de novo confirmada idempotente (0 criadas); dar baixa funcionou e bloqueou segunda tentativa ("Pagamento já quitado"). Dados de teste removidos ao final.

## [Decisão de produto 2026-09-18/19] Comissão da plataforma reformulada: por lado, hierarquia de Agência, bloqueio de estorno pós-fatura
Pedido do usuário: a regra de comissão implementada nos passos anteriores (gerente ganhando % direto do valor da transação) estava conceitualmente errada. A regra real, com exemplo dado pelo usuário: "empresa A vai pagar comissão na venda... empresa B vai pagar comissão na compra... todas cadastradas pela Matriz que cobra 10%... A vendeu a B 1000 RT... Matriz ganha 10% de cada empresa e repassa 5% do ganho da transação pra cada [gerente]". Ou seja: a comissão de gerente é um percentual **em cima da comissão da Agência/Matriz**, não do valor bruto da transação — e essa comissão da própria plataforma passou a poder nascer dos DOIS lados de uma transação (comprador e vendedor), não só do comprador como sempre foi.

Discutido via `/superpowers:brainstorming` em várias rodadas. Pontos fechados:

**1. Comissão da plataforma agora é por lado, não mais um campo único.** `Transacao.comissaoBRL` (campo único, só calculado do lado comprador) foi **removido**. Nova tabela `ComissaoPlataforma` (mesmo espírito de `ComissaoGerente`) registra 0, 1 ou 2 linhas por transação:
- **Associado**: só gera comissão se `tipoOperacao` cobrir o papel exercido (`compra`/`venda`/`compra_venda` — este último com o percentual do PLANO já pela metade, ex: 10% → 5%). Quem recebe: a agência que cadastrou o associado, ou Matriz se cadastro direto.
- **Agência operando na própria conta**: só quando ELA compra (nunca vende — sem `tipoOperacao`, comportamento antigo mantido). Quem recebe: `Agencia.agenciaParenteId` (a agência PAI), ou Matriz se não tiver pai.
- **Matriz**: nunca gera comissão pra si mesma.

**2. Bug corrigido no processo:** antes, quando a própria Agência comprava, a comissão gerada ia creditada **pra ela mesma** (usava `agenciaId` da própria conta) — o repasse nunca de fato acontecia pra ninguém. Agora usa `agenciaParenteId` corretamente: Agência Comum paga a Master que a cadastrou; Master (sem pai) paga Matriz.

**3. Comissão de gerente agora deriva da comissão da plataforma, não do valor bruto.** `comissao_gerente = comissao_da_plataforma_daquele_lado × percentualComissao_do_gerente / 100` — sempre com o percentual do GERENTE cheio (a divisão de `compra_venda` já aconteceu na comissão da plataforma, não se divide duas vezes). `registrarComissoesGerenteDaTransacao()` (`manager.service.ts`) lê as linhas de `ComissaoPlataforma` já criadas e gera uma `ComissaoGerente` por linha com `associadoId` preenchido.

**4. Status + bloqueio de estorno pós-fatura.** `ComissaoPlataforma` e `ComissaoGerente` ganharam `status` (`ativa | estornada`) e uma referência nullable pra consolidação que as incluiu (`cobrancaId`/`pagamentoGerenteId`). Regra do usuário: "não podemos fazer estorno após o fechamento da fatura" — `transactionService.comissaoJaFaturada(transacaoId)` verifica se alguma comissão da transação já tem essas referências preenchidas; se sim, `estorno()` (e `solicitarEstorno()`, pra feedback antecipado) rejeita com erro claro. Se a comissão ainda está solta (mês corrente, não faturada), o estorno segue normal e as linhas viram `estornada` — somem da próxima consolidação.

**5. Tabelas continuam separadas** (`ComissaoPlataforma` × `ComissaoGerente` × `Cobranca` × `PagamentoGerente`) — decisão já tomada e reconfirmada: são ciclos de vida diferentes (recebível com vencimento/RT × pagável sempre BRL sem movimento de saldo).

**Criação passou a ser síncrona:** `ComissaoPlataforma` agora é criada DENTRO da mesma `$transaction` do débito/crédito da permuta/negociada (não mais via fila assíncrona) — garante que comissão e movimento de RT nunca ficam inconsistentes entre si. `ComissaoGerente` continua assíncrona (fila `commission.gerente`), já que deriva de dados já commitados e não precisa da mesma atomicidade.

**Validado:** `npx tsc --noEmit` e `npm test` (36/36) limpos; `npx eslint`/`npm run build` (front) sem erro; contra API/Postgres/Redis reais em Docker, reproduzido o exemplo exato do usuário — Empresa A (venda, gerente A 5%) vende RT$1000 pra Empresa B (compra, gerente B 5%), ambas cadastradas direto pela Matriz (10%): confirmado `ComissaoPlataforma` com duas linhas de R$100 cada, `ComissaoGerente` com R$5 pra cada gerente — bate exatamente com o exemplo. Testado também: associado `compra_venda` comprando de um associado `compra` (que não gera nada vendendo, confirmado) — plataforma nasce em 5% (metade de 10%) = R$25, gerente ganha 5% cheio disso = R$1,25. Testado hierarquia de Agência: Agência Comum comprando da Matriz gera comissão que vai pra Agência Master (o pai), não pra ela mesma. Testado bloqueio de estorno: transação com comissão já consolidada numa Cobranca/PagamentoGerente é rejeitada ("já foi incluída numa fatura fechada"); transação nova (não faturada) foi estornada normalmente e suas comissões viraram `estornada`, confirmadas fora da consolidação seguinte. Dados de teste removidos ao final.

## [RESOLVIDO 2026-09-19] "Próxima fatura" virou valor em tempo real + detalhe das faturas consolidadas
Seguimento do pedido anterior (bloqueio de estorno pós-fatura): o usuário pediu pra construir a visão de detalhe que faltava ("lista as movimentações?") e corrigir o dashboard do associado — "Próxima fatura" tem que ser o VALOR que vai sendo acrescentado em tempo real (não a data), já que representa o que ele vai pagar pra quem o cadastrou (Matriz ou Agência). "Data para Pagamento" já estava correta (data de vencimento da última cobrança real).

**O que mudou:**
- **Novo endpoint `GET /cobrancas/minha-comissao-acumulada`** (qualquer autenticado, usa `request.user.contaId`) — soma `ComissaoPlataforma.comissaoBRL` da própria conta ainda não faturada (`status: ativa`, `cobrancaId: null`). Cresce a cada transação nova, zera quando o job mensal consolida. `ResumoFinanceiro.jsx`: "Próxima fatura" passa a mostrar esse valor (`R$ X`) em vez da data.
- **Novo endpoint `GET /cobrancas/:id/comissoes`** — lista as linhas de `ComissaoPlataforma` que compõem uma `Cobranca` de comissão já consolidada (data, operação, valor da transação, valor da comissão). `ContasModal.jsx` busca isso automaticamente quando abre uma cobrança `tipo: 'comissao'` (lazy, só nesse caso) e mostra a tabela de detalhe.
