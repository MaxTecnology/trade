...
- `POST /associados`: `valorInscricaoRT` (quando informado) não pode ser maior que `limiteCredito` — validado por `.refine()` no Zod schema (`createAssociateSchema`), espelhado no front (`associadoSchema.js`) pra feedback inline. Garante que o débito de inscrição em RT (ver abaixo) nunca viola o CHECK de saldo — associado sempre começa com `saldo = 0`. Vale só na criação — edição posterior de `limiteCredito` (`PUT /associados/:id`) não repete a checagem. Desde 2026-09-08 o formulário de cadastro torna essa checagem inatingível na prática: campo "Limite Crédito" fica travado (`disabled`) e segue automaticamente `valorInscricaoRT`, nunca editável de forma independente.
- **Cobrança de inscrição em RT nasce já quitada** — decisão de produto de 2026-08-27. Diferente da cobrança em BRL (liquidada fora do sistema, fica pendente até reconciliação manual), o RT é movimentado dentro do próprio sistema, então `POST /associados` já debita `valorInscricaoRT` da conta do associado (dentro da mesma transação da criação) e credita a conta da Agência (`input.agenciaId`) ou da Matriz (sem agência) — duas pernas de `MovimentacaoConta`, mesma mecânica de `quitarCobranca()`. A `Cobranca` (`tipo: 'inscricao'`) continua sendo criada, mas já com `pago: true`, só pra manter o histórico de que aquele débito inicial foi uma cobrança de inscrição.
